...
def P0():	# Process 0 controlles servo0
	...
	while True:
		...
		if _Servo0CP < _Servo0DP:					# if Servo0CP less than Servo0DP
			...
			ServoBlaster.write('0=' +str(300 - _Servo0CP)+ '\n')	#
			ServoBlaster.flush()					#
...
def P2():	# Process 0 controlles servo0
	speed = .1		# Here we set some defaults:
	_Servo2CP = 99		# by making the current position and desired position unequal,-
	_Servo2DP = 100	# 	we can be sure we know where the servo really is. (or will be soon)
	while True:
		time.sleep(speed)
		if Servo2CP.empty():			# Constantly update Servo0CP in case the main process needs-
			Servo2CP.put(_Servo2CP)		# 	to read it
		if not Servo2DP.empty():		# Constantly read read Servo2DP in case the main process-
			_Servo2DP = Servo2DP.get()	#	has updated it
		if not Servo2S.empty():			# Constantly read read Servo0S in case the main process-
			_Servo2S = Servo2S.get()	# 	has updated it, the higher the speed value, the shorter-
			speed = .1 / _Servo2S		# 	the wait between loops will be, so the servo moves faster
		if _Servo2CP < _Servo2DP:					# if Servo2CP less than Servo2DP
			_Servo2CP += 1						# incriment Servo2CP up by one
			Servo2CP.put(_Servo2CP)					# move the servo that little bit
			ServoBlaster.write('0=' +str(300 - _Servo2CP)+ '\n')	#
			ServoBlaster.flush()					#
			if not Servo2CP.empty():				# throw away the old Servo2CP value,-
				trash = Servo2CP.get()				# 	it's no longer relevent
		if _Servo2CP > _Servo2DP:					# if Servo2CP greater than Servo2DP
			_Servo2CP -= 1						# incriment Servo2CP down by one
			Servo2CP.put(_Servo2CP)					# move the servo that little bit
			ServoBlaster.write('0=' +str(300 - _Servo2CP)+ '\n')	#
			ServoBlaster.flush()					#
			if not Servo2CP.empty():				# throw away the old Servo2CP value,-
				trash = Servo2CP.get()				# 	it's no longer relevent
		if _Servo2CP == _Servo2DP:	        # if all is good,-
			_Servo2S = 1		        # slow the speed; no need to eat CPU just waiting
			
			
			
def P3():	# Process 0 controlles servo0
	speed = .1		# Here we set some defaults:
	_Servo3CP = 99		# by making the current position and desired position unequal,-
	_Servo3DP = 100		# 	we can be sure we know where the servo really is. (or will be soon)
	while True:
		time.sleep(speed)
		if Servo3CP.empty():			# Constantly update Servo3CP in case the main process needs-
			Servo3CP.put(_Servo3CP)		# 	to read it
		if not Servo3DP.empty():		# Constantly read read Servo3DP in case the main process-
			_Servo3DP = Servo3DP.get()	#	has updated it
		if not Servo3S.empty():			# Constantly read read Servo0S in case the main process-
			_Servo3S = Servo3S.get()	# 	has updated it, the higher the speed value, the shorter-
			speed = .1 / _Servo3S		# 	the wait between loops will be, so the servo moves faster
		if _Servo3CP < _Servo3DP:					# if Servo3CP less than Servo3DP
			_Servo3CP += 1						# incriment Servo3CP up by one
			Servo3CP.put(_Servo3CP)					# move the servo that little bit
			ServoBlaster.write('0=' +str(300 - _Servo3CP)+ '\n')	#
			ServoBlaster.flush()					#
			if not Servo3CP.empty():				# throw away the old Servo3CP value,-
				trash = Servo3CP.get()				# 	it's no longer relevent
		if _Servo3CP > _Servo3DP:					# if Servo3CP greater than Servo3DP
			_Servo3CP -= 1						# incriment Servo3CP down by one
			Servo3CP.put(_Servo3CP)					# move the servo that little bit
			ServoBlaster.write('0=' +str(300 - _Servo3CP)+ '\n')	#
			ServoBlaster.flush()					#
			if not Servo3CP.empty():				# throw away the old Servo3CP value,-
				trash = Servo3CP.get()				# 	it's no longer relevent
		if _Servo3CP == _Servo3DP:	        # if all is good,-
			_Servo3S = 1		        # slow the speed; no need to eat CPU just waiting


Process(target=P0, args=()).start()	# Start the subprocesses
Process(target=P1, args=()).start()	#
Process(target=P2, args=()).start()
Process(target=P3, args=()).start()
time.sleep(2)				# Wait for them to start
...
def GunRev(distance, speed):			# Same logic as above
	global _Servo2CP
	if not Servo2CP.empty():
		_Servo2CP = Servo2CP.get()
	_Servo2DP = distance
	if _Servo2DP < _Servo2LL:
		_Servo2DP = _Servo2LL
	Servo2DP.put(_Servo2DP)
	Servo2S.put(speed)
	return;

def GunFire(distance, speed):			# Same logic as above
	global _Servo3CP
	if not Servo3CP.empty():
		_Servo3CP = Servo3CP.get()
	_Servo3DP = distance
	if _Servo3DP < _Servo3LL:
		_Servo3DP = _Servo3LL
	Servo3DP.put(_Servo3DP)
	Servo3S.put(speed)
	return;
...
while True:
	...
	if not Servo0CP.empty():
		_Servo0CP = Servo0CP.get()
		print str(_Servo0CP) + "  servo position" 
		print str(abs(Cface[0]- _Servo0CP)) + "  servo math" 
		print str(abs(_Servo0CP - Cface[0])) + "  servo math" 
	if (abs(Cface[0]- _Servo0CP) <= 40 or abs(_Servo0CP - Cface[0] ) <= 40)  and faceFound== True:
		print str("fire")
		# os.system("echo 2=2000 > /dev/servoblaster")
		GPIO.output(12, GPIO.HIGH)
		# time.sleep(1.5)
		# GPIO.output(13, GPIO.HIGH)
		# os.system("echo 3=2000 > /dev/servoblaster") 
		# time.sleep(.25)
